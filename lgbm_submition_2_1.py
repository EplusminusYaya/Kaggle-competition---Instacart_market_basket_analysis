...
#  user_id 应该被换为 product_id，'user_id' : 'size' => ‘reordered' : 'count'   
    ...
    #data.columns = ['up_orders', 'up_first_order', 'up_last_order', 'up_mean_cart_position', 'up_median_cart_position',
                    ...
                    #'reordered_sum'
                    #]
    #data.columns = ['up_orders', 'up_first_order', 'up_last_order', 'up_mean_cart_position',
    ...
    data['user_product_reordered_ratio'].fillna(0, inplace = True)
   ...
   ############################
    ...
    orders_train_part, orders_test_part = train_test_split(unique_orders, test_size=0.25, random_state=2017)
    order_test_part = order_train.loc[np.in1d(order_train.order_id, orders_test_part)]
    order_train_part = order_train.loc[np.in1d(order_train.order_id, orders_train_part)]
...
#    categories = ['product_id', 'aisle_id', 'department_id']
    ...
    data_train = order_train_part[features]
    labels_train = order_train_part[['reordered']].values.astype(np.float32).flatten()

    data_val = order_test_part[features]
    labels_val = order_test_part[['reordered']].values.astype(np.float32).flatten()

#    assert data.shape[0] == 8474661

#    lgb_train = lgb.Dataset(data_train, labels_train, categorical_feature=['product_id', 'aisle_id', 'department_id'])
    lgb_train = lgb.Dataset(data_train, labels_train, categorical_feature=['aisle_id', 'department_id'])
#    lgb_eva = lgb.Dataset(data_val, labels_val, reference = lgb_train,  categorical_feature=['product_id', 'aisle_id', 'department_id'])
...
#                    early_stopping_rounds = 10)
    ...
    gbm = lgb.train(params,
                    ...
                    valid_sets = lgb_eval,
                    learning_rates= lambda iter : 0.03 * (0.99 ** iter),
...
#    prediction1 = gbm.predict(data_test)
#    result = pd.DataFrame({'product_id': products, 'order_id': orders, 'prediction': prediction1})
#    result.to_pickle('data/prediction_lgbm_2_1_decay_rate.pkl')
...
#    num_round = gbm.best_iteration
    gbm = lgb.train(params,
                    lgb_train,
                    num_boost_round = 1000,
		    learning_rates= lambda iter : 0.03 * (0.99 ** iter))
   
    data_test = order_test[features]    
    orders = order_test.order_id.values
    products = order_test.product_id.values
    prediction2 = gbm.predict(data_test)
    result = pd.DataFrame({'product_id': products, 'order_id': orders, 'prediction': prediction2})
    result.to_pickle('data/prediction_lgbm_2_2_decay_rate.pkl')
