...
def create_products(df, pNone = None):
   ...
   opt = F1Optimizer.maximize_expectation(P[sort_index], pNone)
   #opt_list = list()
   #opt_list.append(opt[0])
   #opt_list.append(opt[1])
   ...
   #best_prediction = ' '.join(map(lambda x : str(x) if ((not opt_list[1]) & (opt_list[0] != 0)) else 'None', products[0:opt_list[0]])) 
   best_prediction = ['None'] if (opt[1] | (opt[0] == 0)) else []
   best_prediction.extend(products[sort_index][0:opt[0]])
  
   df = df[0:1]
   df.loc[:, 'products'] = ' '.join(str(v) for v in best_prediction)
...
#   data[['order_id', 'products']].to_csv('data/f1_optimal_2/arboretum/arboretum-network_sub.csv', index=False)
